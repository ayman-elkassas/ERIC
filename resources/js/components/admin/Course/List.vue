                ...
                <vs-row justify="flex-end">
                    ...
                    <vs-col style="width:18%" w="2">
                        <vs-tooltip top>
                            ...
                            >
                                <i class='bx bx-trash' ></i> Delete All Courses
                            </vs-button>
                            <template #tooltip>
                                Delete All Courses And Initialize User Role &#128540;
                            </template>
                        </vs-tooltip>
                    </vs-col>
                </vs-row>
                ...
                <vs-table
                    ...
                    <template #tbody>
                        ...
                        >
                            ...
                            <vs-td>
                                {{ tr.course_user.fname }} {{ tr.course_user.lname }}
                            </vs-td>
                    ...
                    <div class="card-body viewPost">
                        <div class="d-flex justify-content-start align-items-center mb-1 mt-0">
                            ...
                            <div class="profile-user-info">
                                <h6 class="mb-0">{{ getAllPosts[index].course_user.fname }} {{ getAllPosts[index].course_user.lname }}</h6>
                                <small class="text-muted">{{getAllPosts[index].created_at}}</small>
                            </div>
                        </div>
                        <p style="color: #7366ff;" class="card-title mt-3">
                            <b>{{ getAllPosts[index].course_field.name }}</b>
                        </p>
                        ...
                        <!-- post img -->
                        <img class="img-fluid rounded mb-3" :src="getAllPosts[index].course_cover" alt="avatar img">
                        <!--/ post img -->
...
export default {
    ...
    computed:{
        getAllPosts(){
            ...
            const posts=this.$store.getters.getAllCourse;
    ...
    methods: {
        createNewUser(){
            ...
            this.$router.push({name: 'course-new', params: { status: 1 } });
        },
        editCurrentUser(id,index){
            //todo:if you want to send params to component in router-link should call as <name> no <path>
            this.$router.push({name: 'course-new', params: { status: 2,id:id,avatar:this.avatars[this.data[index].user_id],data:this.data[index] } });
        },
        ...
        performDelete(){
            ...
            axios.delete('/admin-course/course/'+(this.id)+'?token='+this.authInfo.token+
        ...
        deleteAllCategory(){
            if(localStorage.hasOwnProperty('token')
                ...
                axios.get('/admin-course/remove-all-courses?token='+this.authInfo.token+
                    '&provider='+this.authInfo.provider)
                    .then((response)=>{
                        if(response.data!=="error"){
                            this.loading.close();
                            this.openNotification('top-right',
                                'danger',
                                `<i class='bx bx-select-multiple' ></i>`,
                                "All Courses Are Deleted Successfully",
                                "Can add new role will be able to handle new permission and assign users...");
